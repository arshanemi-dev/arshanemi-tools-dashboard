...
export default function TeamPage() {
  ...
  useEffect(() => {
    fetch('/api/admin/team').then((r) => r.json()).then((d) => { setData(d); setLoading(false) })
  }, [])

  async function handleDelete(item) {
    setDeleting(true)
    await fetch(`/api/admin/team/${item.id}`, { method: 'DELETE' })
  ...
  return (
    <>
      <PageHeader title="Team Members" newHref="/settings/team/new" />
      {loading ? <div className="text-subtle text-sm">Loading…</div> : (
        <DataTable columns={columns} data={data} searchKeys={['name', 'role']}
          actions={(row) => (
            <div className="flex gap-2 justify-end">
              <Link href={`/settings/team/${row.id}`} className="p-1.5 rounded-lg text-subtle hover:text-accent hover:bg-accent/10"><Edit className="w-4 h-4" /></Link>
      ...
      <ConfirmDialog open={!!confirm} title={`Remove "${confirm?.name}"?`} description="This cannot be undone."
