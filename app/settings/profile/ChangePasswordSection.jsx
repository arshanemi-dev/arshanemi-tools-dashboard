...
// have), verified and applied together via /api/auth/change-password.
export default function ChangePasswordSection({ email, mobile, onDone }) {
  ...
  async function handleSave() {
    ...
    if (!codeSent || code.length < 6) { setError('Send and enter the 6-digit code first'); return }
    setSaving(true)
    try {
      const res = await fetch('/api/auth/change-password', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ oldPassword, newPassword, otpCode: code, identifier }),
      })
      const data = await res.json()
